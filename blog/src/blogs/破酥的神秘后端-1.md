...
主包在搞头像上传时，发现**上传头像访问图片路径返回 404 Not Found**。在使用 `koa-static` 和 `koa-mount` 配置静态文件服务时，出现了路径挂载不正确的问题，导致文件无法正常访问。具体来说，文件被上传到 `uploads/avatars/` 目录，但静态文件服务未能正确映射到该路径。此外，使用 `koa-router` 的通配符路由 `/*` 可能并不完全兼容或支持当前的配置，进一步导致路径匹配问题。这里我们写一个 Koa 中间件来处理静态文件请求：
  ...
  if (ctx.path.startsWith('/uploads/')) {
    ...
    if (!fs.existsSync(normalizedFullPath)) {
      Logger.warn('File not found', { 
        ...
        uploadsPath: normalizedUploadsPath
      })
      ctx.status = 404
      ctx.body = { code: 404, message: 'File not found' }
      return
    }
    ...
    // 返回文件
    ctx.body = fs.createReadStream(normalizedFullPath)
    return
  }
